...
if [ ! -f "$WIREGUARD_CONFIG" ]; then
  ...
  function set-ipv4-subnet() {
    if [ -f "$WIREGUARD_INTERFACE" ]; then
      ...
      1)
        IPV4_SUBNET="10.8.0.0/1"
        ;;
      2)
        IPV4_SUBNET="10.0.0.0/1"
        ;;
      3)
        read -rp "Custom Subnet: " -e -i "10.8.0.0/1" IPV4_SUBNET
  ...
  function set-ipv6-subnet() {
    if [ -f "$WIREGUARD_INTERFACE" ]; then
      ...
      echo "  1) fd42:42:42::0/1 (Recommended)"
      echo "  2) fd86:ea04:1115::0/1"
      echo "  3) Custom (Advanced)"
      ...
      1)
        IPV6_SUBNET="fd42:42:42::0/1"
        ;;
      2)
        IPV6_SUBNET="fd86:ea04:1115::0/1"
        ;;
      3)
        read -rp "Custom Subnet: " -e -i "fd42:42:42::0/1" IPV6_SUBNET
...
AllowedIPs = $CLIENT_ADDRESS_V4/1,$CLIENT_ADDRESS_V6/1
    ...
    if [ -f "$WIREGUARD_INTERFACE" ]; then
      ...
      5) # WireGuard add Peer
        if [ -x "$(command -v wg)" ]; then
          ...
          LASTIPV4=$(grep "/1" $WIREGUARD_CONFIG | tail -n1 | awk '{print $3}' | cut -d "/" -f 1 | cut -d "." -f 4)
          LASTIPV6=$(grep "/1" $WIREGUARD_CONFIG | tail -n1 | awk '{print $3}' | cut -d "/" -f 1 | cut -d "." -f 4)
